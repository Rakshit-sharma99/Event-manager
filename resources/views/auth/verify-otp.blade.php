@extends('layouts.guest', ['title' => 'Verify Your Email — Eventra'])
@section('hide-nav', '1')
...
<div class="min-h-screen flex">
    ...
    <div class="hidden lg:flex flex-1 bg-brand-gradient relative overflow-hidden items-center justify-center p-12">
        ...
        <div class="relative z-10 max-w-md text-white text-center">
            ...
            <h2 class="text-h2 font-extrabold mb-4 leading-tight">Securing your journey</h2>
            <p class="text-body-lg opacity-80 leading-relaxed">Let's verify your email address to ensure your account is protected and you receive all event updates.</p>
    ...
    <div class="flex-1 flex items-center justify-center p-6 lg:p-12 bg-white">
        <div class="w-full max-w-md" data-animate="fade-up">
            ...
            <p class="text-body text-surface-500 mb-8">We sent a 6-digit code to <strong class="text-neutral-dark">{{ auth()->user()->email }}</strong></p>

            <form method="POST" action="{{ route('verification.verify-otp') }}" id="otp-form" class="space-y-6">
                @csrf
                ...
                <!-- Hidden field that JS populates with joined OTP -->
                ...
                <button type="submit" class="btn-primary w-full py-3" id="otp-submit-btn">Verify Account</button>
            </form>
            ...
            <form method="POST" action="{{ route('logout') }}" class="mt-8 text-center text-body">
                @csrf
                <button type="submit" class="font-semibold text-surface-500 hover:text-primary-500 underline transition-colors">Use a different account</button>
            </form>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ── Auto-focus & advance between inputs ──
    ...
    let remaining = {{ auth()->user()->otp_expires_at ? max(0, auth()->user()->otp_expires_at->diffInSeconds(now())) : 900 }};
    ...
    @if(auth()->user()->otp_last_resent_at)
        resendCooldown = Math.max(0, 60 - {{ now()->diffInSeconds(auth()->user()->otp_last_resent_at) }});
    @endif
