...
<section class="auth-stage">
    ...
    <div class="auth-card">
        ...
        <p class="plain-muted">We sent a 6-digit code to <strong>{{ auth()->user()->email }}</strong></p>
        ...
        <form method="POST" action="{{ route('verification.verify-otp') }}" id="otp-form">
            ...
            @if($errors->has('otp_code'))
                <p class="otp-error">{{ $errors->first('otp_code') }}</p>
            @endif
            ...
            <button class="auth-submit" type="submit" id="otp-submit-btn">Verify Account</button>
        </form>
        ...
        <form method="POST" action="{{ route('logout') }}" style="margin-top: 16px; text-align: center;">
            @csrf
            <button type="submit" style="background: none; border: none; color: #0645ad; cursor: pointer; text-decoration: underline;">Use a different account</button>
        </form>
    </div>
</section>
