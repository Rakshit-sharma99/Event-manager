...
<div class="relative min-h-screen flex items-center justify-center overflow-hidden py-8 bg-black">
    ...
    <div class="relative z-10 w-full max-w-md mx-4" data-animate="fade-up">
        <div class="backdrop-blur-xl bg-white/10 border border-white/15 rounded-2xl shadow-[0_8px_60px_rgba(0,0,0,0.5)] p-8 md:p-10">
            {{-- Logo --}}
            <a href="{{ route('landing') }}" class="flex items-center justify-center gap-2 text-h3 font-extrabold text-white mb-6">
                <svg class="w-8 h-8" viewBox="0 0 24 24" fill="none">
                    ...
                    <path d="M12 2C12 7.5 16.5 12 22 12C16.5 12 12 16.5 12 22C12 16.5 7.5 12 2 12C7.5 12 12 7.5 12 2Z" fill="none" stroke="url(#reg-logo-grad)" stroke-width="2.5" stroke-linejoin="round"/>
                </svg>
                Eventra
            </a>
            ...
            <form method="POST" action="{{ route('register.store') }}" class="space-y-4">
                ...
                <div x-data="{ pass: '', strength: 0, show: false }" class="space-y-1.5">
                    <label for="reg-password" class="block text-body font-medium text-white/80">Password <span class="text-red-400">*</span></label>
                    <div class="relative">
                        <input
                        ...
                        >
                        <button type="button" @click="show = !show" class="absolute right-3 top-1/2 -translate-y-1/2 text-white/40 hover:text-white/60">
                    ...
                    <div class="flex gap-1 mt-2" x-show="pass.length > 0">
                        <template x-for="i in 4">
                            <div class="h-1 flex-1 rounded-full transition-colors duration-300"
                                 :class="i <= strength ? (strength <= 1 ? 'bg-red-400' : strength <= 2 ? 'bg-amber-400' : 'bg-green-500') : 'bg-white/10'"></div>
                        </template>
                    </div>
                ...
                <div class="space-y-1.5">
                    <label for="reg-password-confirm" class="block text-body font-medium text-white/80">Confirm Password</label>
                    <input id="reg-password-confirm" name="password_confirmation" type="password" placeholder="••••••••" required
                           class="w-full px-4 py-3 rounded-xl text-body bg-white/10 border border-white/15 text-white placeholder:text-white/30 focus:border-primary-400 focus:ring-2 focus:ring-primary-500/20 focus:outline-none transition-all backdrop-blur-sm">
                </div>

                {{-- Role Selection --}}
                <div class="space-y-1.5">
                    <label class="block text-body font-medium text-white/80">I am a <span class="text-red-400">*</span></label>
                    <div class="grid grid-cols-3 gap-3" x-data="{ role: '{{ old('role', 'planner') }}' }">
                        @foreach(['planner' => '📋 Planner', 'vendor' => '🏪 Vendor', 'guest' => '🎟️ Guest'] as $value => $label)
                            <label @click="role = '{{ $value }}'" :class="role === '{{ $value }}' ? 'border-primary-400 bg-primary-500/20 text-white' : 'border-white/15 text-white/60 hover:border-white/30'"
                ...
                <button type="submit" class="w-full py-3 rounded-xl bg-gradient-to-r from-primary-500 via-secondary-500 to-accent text-white font-bold text-body hover:shadow-glow hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200">Create My Account ✦</button>
            </form>
            ...
            <p class="mt-6 text-center text-body text-white/50">
                Already have an account? <a href="{{ route('login') }}" class="font-semibold text-primary-400 hover:text-primary-300 transition-colors">Sign in</a>
            </p>
        </div>
    </div>
</div>
