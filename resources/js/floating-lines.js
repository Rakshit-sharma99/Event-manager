/**
 * FloatingLines — Vanilla JS port of ReactBits FloatingLines component
 * Uses Three.js for WebGL shader-based animated floating lines.
 */
import {
  Clock,
  Mesh,
...
/**
 * Mount FloatingLines WebGL animation onto a container element.
 * @param {HTMLElement} container - The DOM element to render into
 * @param {Object} options - Configuration options
 * @returns {Function} cleanup function to destroy the renderer
 */
export default function mountFloatingLines(container, options = {}) {
  const {
    linesGradient,
  ...
  const gradient = linesGradient || [gradientStart, gradientMid, gradientEnd];
  ...
  const topLD = enabledWaves.includes('top') ? (typeof lineDistance === 'number' ? lineDistance : getLineDistance('top')) * 0.01 : 0.01;
  const middleLD = enabledWaves.includes('middle') ? (typeof lineDistance === 'number' ? lineDistance : getLineDistance('middle')) * 0.01 : 0.01;
  const bottomLD = enabledWaves.includes('bottom') ? (typeof lineDistance === 'number' ? lineDistance : getLineDistance('bottom')) * 0.01 : 0.01;
